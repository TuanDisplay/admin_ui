...
import type { IIdeaDeMana } from "~/common/types";
...
function IdeaDeManaUI({ idea_id }: { idea_id: string }) {
  const { data } = useQuery({
    ...
    queryFn: async (): Promise<IIdeaDeMana> => {
      const res = await productService.ideaWattingDe(idea_id);
      return res.data;
    },
  });

  // console.log("Data: " + JSON.stringify(dataId));
    ...
    <div className="max-w-5xl mx-auto p-6 space-y-6">
      ...
      <div>
        ...
        <p
          ...
          dangerouslySetInnerHTML={convertStringToHtml(data?.content_detail)}
        ></p>
      </div>
      ...
      <div>
        ...
        <p
          className="text-gray-800 whitespace-pre-line "
          dangerouslySetInnerHTML={convertStringToHtml(data?.value_benefits)}
        ></p>
      </div>
