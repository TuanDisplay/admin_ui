...
function ProblemDeManaUI({ problem_id }: { problem_id: string }) {
  const { data, isLoading } = useQuery({
    queryKey: ["problemDeMana", problem_id],
    queryFn: async (): Promise<IProductDeMana> => {
      const res = await productService.problemWattingDe(problem_id);
      return res.data;
    },
  });
...
export default function ProblemDeMana() {
  ...
  return <ProblemDeManaUI problem_id={problemId} />;
}
